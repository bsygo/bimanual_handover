...
class CloseContactMover():
    ...
    def __init__(self):
        self.joints = ['ffj2', 'mfj2', 'rfj2', 'lfj2', 'thj5']
        self.joint_publishers = {}
        for joint in self.joints:
            self.joint_publishers[joint : rospy.Publisher('sh_rh_' + joint + '_position_controller/command', Float64, queue_size = 1)]
        self.biotac_subs = [rospy.Subscriber('rh/biotac_' + joint, Biotac, callback = self.biotac_callback, callback_args = joint, queue_size = 10) for joint in self.joints]
        ...
        self.current_joint_values = [0, 0, 0, 0, 0]
        self.initial_biotac_values = [0, 0, 0, 0, 0]
        self.biotac_threshold = 5
    ...
    def joint_callback(self, joint_state):
        indices = [joint_state.name.index('rh_FFJ2'), joint_state.name.index('rh_MFJ2'), joint_state.name.index('rh_RFJ2'), joint_state.name.index('rh_LFJ2'), joint_state.name.index('rh_THJ5')]
    ...
    def move_until_contacts(self):
        ...
        finished = False
        targets = [Float64(1.57), Float64(1.57), Float64(1.57), Float64(1.57), Float64(1.0)]
